# booster-um-artix
Booster UKI Manager - A simple bash script to manage UKI (Unified Kernel Image) files generated by booster and eukify.
The script can only be used on Artix Linux.
...
If you are using an Artix, install [booster-um](https://aur.archlinux.org/packages/booster-um) or [booster-um-git](https://aur.archlinux.org/packages/booster-um-git) package from the AUR. After that you are free to remove `mkinitcpio`.

<h2 name="default_config">
    Default configuration
</h2>

* booster-um-artix by default:
  * Signs generated UKI files with `sbctl` **if installed**
  * Uses default splash `/usr/share/egummiboot/bootctl/splash-artix.bmp`
  ...
  * Removes known EFI entries (Example: `\EFI\LINUX\ARTIX-LINUX.EFI`) **if efibootmgr is installed**. To prevent this, enable `efistub` in config

* booster-um-artix libalpm hooks by default:
...
* `egummiboot` will, if installed, detect UKI files generated by `booster-um`. For more information about booting, see the [Unified kernel image - Booting](https://wiki.archlinux.org/title/Unified_kernel_image#Booting) article
...
Manage UKI files generated by booster and eukify.
